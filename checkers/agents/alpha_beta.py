...
class MinimaxPlayer(Player):
    ...
    def __init__(
        ...
        self.rollout_order = rollout_order_gen or (
            lambda moves: self.random.permutation(np.asarray(moves, dtype="int,int"))
        )
...
def material_value_adv(color, king_value, man_value, board, turn, last_moved_piece):
    black_adv = material_value(king_value, man_value, board["black"]) - material_value(
        king_value, man_value, board["white"]
    )
...
if __name__ == "__main__":
    ...
    for i in range(n_matches):
        ...
        white_player = MinimaxPlayer(
            "white",
            value_func=partial(material_value_adv, "white", 2, 1),
            search_depth=4,
            seed=i * 2,
        )
        white_player = RandomPlayer("white", seed=i * 2)
        winner = play_a_game(
            ch, black_player.next_move, white_player.next_move, max_game_len
        )
